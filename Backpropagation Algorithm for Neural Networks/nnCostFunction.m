...
XNew=[ones(m,1) X];
for i=1:m
    alpha2=sigmoid(Theta1*XNew(i,:)');
    alpha2New=[1;alpha2];
    hyp=sigmoid(Theta2*alpha2New);
    yNew=zeros(num_labels,1);
    yNew(y(i))=1;    
    for j=1:num_labels
        val=-(yNew(j)*log(hyp(j)))-((1-yNew(j))*log(1-hyp(j)));
        J=J+val;
    end
end
J=J/m;
for i=1:hidden_layer_size
    for j=1:input_layer_size
        val2=(Theta1(i,j+1)^2)*lambda/(2*m);
        J=J+val2;
    end
end
for i=1:num_labels
    for j=1:hidden_layer_size
        val3=(Theta2(i,j+1)^2)*lambda/(2*m);
        J=J+val3;
    end
end
for t=1:m
    ...
    yNew(y(t))=1;        
    sDelta_3=a_3-yNew;
    sDelta_2=(Theta2(:,2:end)'*sDelta_3).*sigmoidGradient(Theta1*a_1');
    Theta2_grad=Theta2_grad+sDelta_3*a_2';
    Theta1_grad=Theta1_grad+sDelta_2*a_1;
end
